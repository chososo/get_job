...
FIELDS={'id','sourceId','sourceUrl','applicationUrl','title','company','category','country','level','eligibility','languageStatus','visaStatus','postedAt','startDate','deadlineDate','deadlineAt','deadlineTimezone','deadlineKind','deadlineText','status','evidence','lastVerifiedAt'}
CATEGORIES={'퀀트 리서치','자산운용·자산배분','ETF·인덱스','금융리스크','파생상품평가'}
def validate(job):
    ...
    if job.get('status') not in ('open','review','closed','upcoming'):raise ValueError('Invalid status')
    if any(isinstance(v,str) and len(v)>1000 for v in job.values()):raise ValueError('Only concise public facts, not full descriptions, are allowed')
    ...
    if job.get('status')=='open' and (job.get('level') not in ('인턴','신입') or not job.get('evidence')):raise ValueError('Open jobs require junior and verification evidence')
